...
스토리지 암.복호화
네이티브 코드의 라이브러리 로딩 부분에 대한 심화 학습 및 직접 구현

HTB 
방향성 체크 문제 풀이를 넘어 터널링 매커니즘 선행 학습 필요 
- 터널링 이후, 내부망 내 취약한 서비스를 CVE를 통해 공략하는 시나리오 위주로 학습 비중 

mitre aws 서비스 별 특징 및 보안 설정 파악 + AND 영어 문법에 대한 공부(인프런) OR 다른 사이트 확인 
